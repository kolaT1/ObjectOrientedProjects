...
public class BinaryTree{
    ...
    static boolean findAncestors(Node node, int find, int[] ancestors) {
        ...
        if (findAncestors(node.left, find, ancestors)
        || findAncestors(node.right, find, ancestors))
        {
            ancestors[ancestorsCount++] = node.val;
            return true;
        } 
        return false;
    }
    ...
    public static void main(String[] args) {
        ...
        while(true){
            ...
            switch(choice){
                ...
                break;
                case'4':
                System.out.println("Number of nodes in the tree are "+btt.countNodes(btt.root,0));

                break;
                case'5':
                System.out.println("Height of the tree is "+btt.treeHeight(btt.root));
                ...
                int[] ancestors = new int [btt.countNodes(btt.root,0)];
                boolean item2 = btt.containsNode(second);
                if (item2) {
                    if (btt.containsNode(first)) {
                        boolean itemAncestor = false;
                        ancestorsCount = 0;
                        btt.findAncestors(BinaryTree.root, second, ancestors);
                        for (int i = 0; i < ancestorsCount; i++) {
                            if (ancestors[i] == first) {
                                itemAncestor = true;
                            }
                        }
                        if (itemAncestor) {
                        ...
                        System.out.println(first + " is not present in binary tree.");
                    }
                }
